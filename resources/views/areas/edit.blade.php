            ...
            <div class="card-body">
                <form action="{{ route('areas.update', $area) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="branch_id" id="branch_id" class="form-control @error('branch_id') is-invalid @enderror" required>
                            ...
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" {{ old('branch_id', $area->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->nama }}</option>
                            @endforeach
                        </select>
                        @error('branch_id')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="kode_area" id="kode_area" class="form-control @error('kode_area') is-invalid @enderror" value="{{ old('kode_area', $area->kode_area) }}" required>
                        @error('kode_area')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="3" class="form-control @error('keterangan') is-invalid @enderror">{{ old('keterangan', $area->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
